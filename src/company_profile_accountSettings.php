...
if( isset($_SESSION['company_id'])) {
	header("Location: ");
       ...
       $records = $conn->prepare('select password from company where company_id = :company_id'); // = ' .$_SESSION['company_id'].);
...
if(  !empty($_POST['password_new'])&& !empty($_POST['password_r_new'])&& ($_POST['password_new']==$_POST['password_r_new'])):
    ...
    // adding to player table
     $records1 = $conn->prepare('UPDATE company SET password = :new_password WHERE company_id = :company_id');
