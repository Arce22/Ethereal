...
if(!empty($_POST['approve_comment']) && !empty($_POST['comment_id']))
{
    $records = $conn->prepare(' UPDATE comment SET approval_status = 1  WHERE comment_id = :comment_id'); // = ' .$_SESSION['company_id'].);
    ...
    echo 'alert("Successfully Comment Disapproved")';
    echo '</script>';
}

if(!empty($_POST['disapprove_comment']) && !empty($_POST['comment_id']))
{
    $records = $conn->prepare(' UPDATE comment SET approval_status = 2  WHERE comment_id = :comment_id'); // = ' .$_SESSION['company_id'].);
    $records->bindParam(':comment_id', $_POST['comment_id']);
    $records->execute();
...
<div>
    ...
    <label> Comment List: </label></br>

    <select name="comment_id" multiple method="post">

            <?php

            $records = $conn->prepare('select * from comment where approval_status = 0 '); // = ' .$_SESSION['company_id'].);
            $records->execute();
            $results = $records->fetchAll(); //(PDO::FETCH_ASSOC)

            foreach($results as $result){
                echo "<option name =\"".$result['comment_id']."\"> ".$result['comment_id']."</option></br></br>";
            }
            ?>

        </select></br></br>

        <input type='submit' name='approve_comment' value='Approve'></br></br>
        <input type='submit' name='disapprove_comment' value='Disapprove'>
    </span>
