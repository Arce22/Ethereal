...
if(!empty($_POST['event_id']) && !empty($_POST['company_id']) )
{
    $records = $conn->prepare('delete from discount where company_id = :company_id and event_id = :event_id'); // = ' .$_SESSION['company_id'].);
    ...
    echo 'alert("Successfully Event Deleted From Discount")';
    echo '</script>';
}

if(!empty($_POST['event_id']) && !empty($_POST['company_id']) )
{
    $records = $conn->prepare('delete from event where event_id = :event_id'); // = ' .$_SESSION['company_id'].);
    //$company = "a" ;
    $records->bindParam(':event_id' ,$_POST['event_id']);
    $records->execute();
    ...
    echo '<script language="javascript">';
    echo 'alert("Successfully Event Deleted From Event")';
    echo '</script>';
}
...
<div>
    ...
    <select name="event_id" multiple method="post">

            <?php

            $records = $conn->prepare('select * from event'); // = ' .$_SESSION['company_id'].);
            $records->execute();
            $results = $records->fetchAll(); //(PDO::FETCH_ASSOC)

            foreach($results as $result){
                echo "<option name =\"".$result['event_id']."\"> ".$result['event_id']."</option></br></br>";
            }
            ?>

        </select></br></br>

        <input type='submit' name='delete_event' value='Delete'></br></br>
    </span>
