...
<?php
      ...
      $records = $conn->prepare('select count(distinct player_id) as number from played where game_name = :game_name'); 
...
<label><?php echo $results['number'];?></label><br />
 ...
 <?php
      ...
      $records = $conn->prepare('select count(distinct player_id) as number from bought where game_name = :game_name'); 
      $records->bindParam(':game_name', $game_name);
      $records->execute();
      $results = $records->fetch(PDO::FETCH_ASSOC);
?>
...
<label><?php echo $results['number'];?></label><br />
   ...
   <?php
      ...
      $records = $conn->prepare('select count(distinct player_id) as females from information natural join played where gender = \'female\' and game_name = :game_name'); 
      $records->bindParam(':game_name', $game_name);
      $records->execute();
      $results = $records->fetch(PDO::FETCH_ASSOC);
?>
...
<label><?php echo $results['females'];?></label><br />
 ...
 <?php
      ...
      $records = $conn->prepare('select count(distinct player_id) as males from information natural join played where gender = \'male\' and game_name = :game_name'); 
      $records->bindParam(':game_name', $game_name);
      $records->execute();
      $results = $records->fetch(PDO::FETCH_ASSOC);
?>
...
<table align = "center">
   ...
   <thead>
    ...
    <tbody>
        ...
        $records = $conn->prepare('SELECT distinct country from information natural join played WHERE game_name = :game_name');
        $records->bindParam(':game_name', $game_name);
        $records->execute();
